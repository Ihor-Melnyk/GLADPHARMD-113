...
function onBeforeCardSave() {
  ...
  if (EdocsApi.getAttributeValue("DocKind").text == "Заявка на внесення змін") auditCounterpartyManagerTable();
}
    ...
    if (method == "edocsCreateDoc") {
      accounts.forEach(account => {
        result.push([
          ...
          { code: "numCode ", value: EdocsApi.findElementByProperty("code", "Currency", account)?.itemCode },
        ]);
      });
    } else {
      accounts.forEach(account => {
...
function onChangeadditionalInfo() {
  if (EdocsApi.getEmployeeDataByEmployeeID(CurrentUser.employeeId).unitId != 177 || EdocsApi.getEmployeeDataByEmployeeID(CurrentUser.employeeId).unitId != 181) throw "Немає прав на редагування поля 'Додаткова інформація'";
}

function onTaskExecuForReview(routeStage) {
  if (routeStage.executionResult == "executed") {
    var nonResident = EdocsApi.getAttributeValue("nonResident");
...
function onChangepayerVAT() {
  if (EdocsApi.getAttributeValue("payerVAT").value == "true") {
