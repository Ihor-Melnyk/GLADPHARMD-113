...
function auditCounterpartyManagerTable(){
    ...
    if(table && table.length > 0){
        if(table.flat().filter(x=>x.value == null || x.value == '').find(y=>y.code == 'counterpartyManagerEmail' || y.code == 'counterpartyManagerPhone' || y.code == 'counterpartyManagerName' || y.code == 'counterpartyManagerPosition'))
            throw 'Перевірте заповнення обов’язкових полів Таблиці Менеджери контрагентів';
    }
    }


function clearAttribute(attributeCode, doNotClearOnInit, isDictionary) { //очищення
    if (doNotClearOnInit) {
        return;
    }
...
function setPropertyRequired(attributeName, boolValue) { //обов"язкове
...
function setPropertyHidden(attributeName, boolValue) { //приховане
	var attributeProps = EdocsApi.getControlProperties(attributeName);
	attributeProps.hidden = boolValue;
	EdocsApi.setControlProperties(attributeProps);
}

function setPropertyDisabled(attributeName, boolValue) { //недоступне
	var attributeProps = EdocsApi.getControlProperties(attributeName);
...
function onTaskExecutecreateCTRPT(routeStage) {
    if (routeStage.executionResult == 'executed') {
        ...
        methodData.attributeValues.push({ "code": "additionalInfo", "value": EdocsApi.getAttributeValue('additionalInfo').value });
...
function onTaskExecuteUpdateCTRPT(routeStage) {
    if (routeStage.executionResult == 'executed') {
        ...
        var methodData = {
            ...
            externalDocId: EdocsApi.getAttributeValue('ContractorID').value/*CurrentDocument.id.toString()*/,
        ...
        methodData.attributeValues.push({ "code": "id", "value": EdocsApi.getAttributeValue('ContractorID').value });
        methodData.attributeValues.push({ "code": "createdById", "value": EdocsApi.getEmployeeDataByEmployeeID(CurrentDocument.initiatorId).extId });
...
function getAccountsTable(accounts, method){
    var result = [];
...
function getPersonsTable(persons, method){
    debugger;
    var result = [];
...
function getContactsTable(method){
    var result = [];
...
function getPostalAddressesTable(addresses, method){
    debugger;
    var result = [];
...
function onChangecounterpartyCode(){
	if(EdocsApi.getAttributeValue('ContractorIsResident').value == 'false')
		isNumbercounterpartyCode();
}
...
function isNumbercounterpartyCode(){
	...
	if(counterpartyCode)
		if(!(/^\d+$/.test(counterpartyCode)))
			throw 'Для резидентів України в полі Код ЄДРПОУ може бути тільки число';
...
function onChangeContractorID() {
    ...
    if (ContractorID.value) {
		...
		EdocsApi.setAttributeValue({code: 'foreignShortName', value: ContractorData.data.ShortNameEN});
        EdocsApi.setAttributeValue({code: 'counterpartyFullName', value: ContractorData.data.FullName});
		EdocsApi.setAttributeValue({code: 'foreignFullName', value: ContractorData.data.FullNameEN});
...
function setTypeOfOwnership(typeOfOwnership) {
    ...
    switch (typeOfOwnership) {
        case 'Юридична особа': return 'LEGAL_ENTITY';
        case 'Фізична особа-підприємець': return 'ENTREPRENEUR';
        case 'Приватна особа': return 'PERSON';
        case 'LEGAL_ENTITY': return 'Юридична особа';
        case 'ENTREPRENEUR': return 'Фізична особа-підприємець';
        case 'PERSON': return 'Приватна особа';
